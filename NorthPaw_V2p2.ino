...
void setup() {
  ...
  // the previous function setMagGain has been eliminated from the library, along with the magGain enum
  // see https://github.com/pololu/lsm303-arduino/commit/b935b0e20ffbc3aa4975bbcf267dfd9a30fc255d
  ...
  // so, was using LSM303::magGain_40, now just use 0x80
  // and must supply register number now, it's CRB_REG_M
  compass.writeReg(LSM303::CRB_REG_M, 0x80);  // set to mid-sensitivity  
...
{ // function which monitors the compass for it being turned upside
  ...
  int threshold = 8000;  // was 800, changed with update to 
  // LSM303 library between 2013 and 2015, not sure WHY but
  // it's probably a placement of LSB issue?  
  // used to be that the y reading was maybe 1500, whereas now it's 15000
  ...
  Serial.println(compass.a.z);
   // in typical orientation, gravity is in y data
  if(compass.a.y > threshold)
    Serial.println("Right Side Up!");
  else if (compass.a.y < -threshold)
...
{  // function to recalibrate the compass, store the results in EEPROM
  ...
  LSM303::vector<int> running_min = {2047, 2047, 2047}, running_max = {-2048, -2048, -2048};
...
{  // does fancy mod averaging routine in order to get reasonable
  ...
  int average = 0;
  compass.read();
  int oldd = compass.heading((LSM303::vector<int>){1,0,0});
  int newd = oldd;
  ...
  for (int i = 0; i<n; i++)
  {
    ...
    compass.read();
    newd = compass.heading((LSM303::vector<int>){1,0,0});
    //Serial.println(newd);
